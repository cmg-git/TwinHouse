...
% 
% Inputs
% TCd   cell array of Wall1bc objects [A,G,b,C,f,y] of each TC
% AssX  assembly matrix: 4 elem/row = TC#node TC#node (2nd node collapse in 1st)
%
% Outputs
% TCa   cell array of [A,G,b,C,f,y] of assembled /TC, T, Q/
% Idx   Idx{1}: nodes; Idx{2}: branches; 
%       1st row: the local indexes; 2nd row: global indexes

T_sources = [];
Q_sources = [];
for i = 1:size(TCd, 2)
    TCdo{i} = TCd{i}.TC;
    T_sources = [T_sources; TCd{i}.T];
    Q_sources = [Q_sources; TCd{i}.Q];
end
% u = [T_sources; Q_sources];
TCd = TCdo;
...
thidx = [thl; thag(:,2)'];  % temp 1st row: local indexes; 2nd row global idx of assembled
qidx = [ql; qag];           % flow 1st row: local indexes; 2nd row global idx of assembled
Idx = {thidx,qidx};

TCa = ThermCirc(TCa, T_sources, Q_sources);
